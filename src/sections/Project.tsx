...
const Project = ({ index, project, selectedProject, onMouseEnter }: Props) => {
    ...
    const handleMouseEnter = contextSafe?.(() => {
        ...
        const boxLength = box?.getTotalLength() || 0;
        const arrowLineLength = arrowLine?.getTotalLength() || 0;
        const arrowCurbLength = arrowCurb?.getTotalLength() || 0;
    ...
    return (
        ...
        >
            {selectedProject === null && (
                <div className="w-full aspect-video mb-6 rounded-xl border border-border/20 shadow-sm overflow-hidden">
                    <img
                        src={project.longThumbnail}
                        alt={project.title}
                        className="w-full h-full object-cover object-top animate-scroll-preview"
                        key={project.slug}
                        loading="lazy"
                    />
                </div>
